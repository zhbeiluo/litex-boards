...
from litex_boards.platforms import xilinx_vcu37p
...
class _CRG(Module):
    def __init__(self, platform, sys_clk_freq, ddram_channel, with_hbm):
        ...
        if with_hbm:
            ...
            pll.create_clkout(self.cd_idelay, sys_clk_freq*4) #, with_reset=False
...
class BaseSoC(SoCCore):
    def __init__(self, 
        ...
        ddram_channel=0, 
        uart_name="serial",
        with_pcie=False, with_led_chaser=False, with_hbm=False, **kwargs):
        platform = xilinx_vcu37p.Platform()
        if with_hbm:
        ...
        # SoCCore ----------------------------------------------------------------------------------
        SoCCore.__init__(self, platform, sys_clk_freq, uart_name=uart_name, ident="LiteX SoC on VCU37P", **kwargs)
        ...
        # UART -------------------------------------------------------------------------------------
        # if with_uart:
        #     self.add_uartbone(name="serial", cd="uart", clk_freq="125e6")
        ...
        # UART Sel
        uart_sel = self.platform.request("uart_sel")
        self.comb += [
            uart_sel.eq(1),
        ]
...
def main():
    ...
    parser = LiteXSoCArgumentParser(description="LiteX SoC on VCU37p")
    target_group = parser.add_argument_group(title="Target options")
    target_group.add_argument("--build",           action="store_true", help="Build design.")
    target_group.add_argument("--load",            action="store_true", help="Load bitstream.")
    target_group.add_argument("--sys-clk-freq",    default=125e6,       help="System clock frequency.") # HBM2 with 250MHz, DDR4 with 150MHz (1:4)
